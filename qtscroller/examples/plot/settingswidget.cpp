...
struct MetricItem
{
    QtScrollerProperties::ScrollMetric metric;
    const char *name;
    int scaling;
    const char *unit;
    QVariant min, max;
...
public:
        ...
        , m_widget(0)
    {
        if (m_item->min.type() == QVariant::PointF) {
            ...
            m_slider->setMaximum(qRound((m_item->max.toPointF().x() - m_item->min.toPointF().x()) / m_item->step.toReal()));
            m_slider->setValue(-1);
            m_slider2 = new QSlider(Qt::Horizontal);
            m_slider2->setSingleStep(1);
            m_slider2->setMinimum(-1);
            m_slider2->setMaximum(qRound((m_item->max.toPointF().y() - m_item->min.toPointF().y()) / m_item->step.toReal()));
    ...
    void setScroller(QWidget *widget)
    {
        ...
        QtScrollerProperties properties = QtScroller::scroller(widget)->scrollerProperties();
...
private slots:
    void valueChanged(const QVariant &v)
    {
        ...
        case QVariant::PointF: {
            m_slider->setValue(qRound((m_value.toPointF().x() * m_item->scaling - m_item->min.toPointF().x()) / m_item->step.toReal()));
            m_slider2->setValue(qRound((m_value.toPointF().y() * m_item->scaling - m_item->min.toPointF().y()) / m_item->step.toReal()));
            break;
        }
        default: break;
        }
    ...
    void sliderChanged(int value)
    {
        ...
        if (m_widget && QtScroller::scroller(m_widget)) {
            QtScrollerProperties properties = QtScroller::scroller(m_widget)->scrollerProperties();
            properties.setScrollMetric(m_item->metric, m_value);
            QtScroller::scroller(m_widget)->setScrollerProperties(properties);
        }
...
#define METRIC(x) QtScrollerProperties::x, #x

MetricItem items[14] = {
    { METRIC(DragVelocitySmoothingFactor),    1,    "",         qreal(0), qreal(1), qreal(0.01) },
    { METRIC(DecelerationFactor),             1,    "",         qreal(0), qreal(3), qreal(0.01) },

    { METRIC(SnapTime),                       1,    "s",        qreal(0), qreal(10), qreal(0.1) },

    { METRIC(OvershootTime),                  1,    "s",        qreal(0), qreal(10), qreal(0.1) },

    { METRIC(OvershootDragResistanceFactor),  1,    "",         qreal(0), qreal(1), qreal(0.01) },
    { METRIC(OvershootMaximumDistanceFactor), 1,    "",         qreal(0), qreal(1), qreal(0.01) },
    ...
    { METRIC(AcceleratingFlickMaximumTime),   1000, "ms",       qreal(10), qreal(1000), qreal(1) },
    ...
    , m_smallscreen(smallscreen)
{
    ...
    m_hospolicy->addItem("When Scrollable", QtScrollerProperties::OvershootWhenScrollable);
    m_hospolicy->addItem("Always On",       QtScrollerProperties::OvershootAlwaysOn);
    m_hospolicy->addItem("Always Off",      QtScrollerProperties::OvershootAlwaysOff);
    ...
    m_vospolicy->addItem("When Scrollable", QtScrollerProperties::OvershootWhenScrollable);
    m_vospolicy->addItem("Always On",       QtScrollerProperties::OvershootAlwaysOn);
    m_vospolicy->addItem("Always Off",      QtScrollerProperties::OvershootAlwaysOff);
    ...
    m_scrollingCurve = new QComboBox();
    m_scrollingCurve->addItem("OutQuad", QEasingCurve::OutQuad);
    m_scrollingCurve->addItem("OutCubic", QEasingCurve::OutCubic);
    m_scrollingCurve->addItem("OutQuart", QEasingCurve::OutQuart);
    m_scrollingCurve->addItem("OutQuint", QEasingCurve::OutQuint);
    m_scrollingCurve->addItem("OutExpo", QEasingCurve::OutExpo);
    m_scrollingCurve->addItem("OutSine", QEasingCurve::OutSine);
    m_scrollingCurve->addItem("OutCirc", QEasingCurve::OutCirc);
    connect(m_scrollingCurve, SIGNAL(currentIndexChanged(int)), this, SLOT(scrollingCurveChanged(int)));
    addToGrid(grid, new QLabel("Scrolling Curve"), 1, m_scrollingCurve);

    m_overshootCurve = new QComboBox();
    m_overshootCurve->addItem("OutQuad", QEasingCurve::OutQuad);
    m_overshootCurve->addItem("OutCubic", QEasingCurve::OutCubic);
    m_overshootCurve->addItem("OutQuart", QEasingCurve::OutQuart);
    m_overshootCurve->addItem("OutQuint", QEasingCurve::OutQuint);
    m_overshootCurve->addItem("OutExpo", QEasingCurve::OutExpo);
    m_overshootCurve->addItem("OutSine", QEasingCurve::OutSine);
    m_overshootCurve->addItem("OutCirc", QEasingCurve::OutCirc);
    connect(m_overshootCurve, SIGNAL(currentIndexChanged(int)), this, SLOT(overshootCurveChanged(int)));
    addToGrid(grid, new QLabel("Overshooting Curve"), 1, m_overshootCurve);
...
void SettingsWidget::setScroller(QWidget *widget)
{
    ...
    m_hospolicy->setCurrentIndex(m_hospolicy->findData(properties.horizontalOvershootPolicy()));
    m_vospolicy->setCurrentIndex(m_vospolicy->findData(properties.verticalOvershootPolicy()));

    m_scrollingCurve->setCurrentIndex(m_scrollingCurve->findData(properties.scrollMetric(QtScrollerProperties::ScrollingCurve).value<QEasingCurve>().type()));
    m_overshootCurve->setCurrentIndex(m_overshootCurve->findData(properties.scrollMetric(QtScrollerProperties::OvershootCurve).value<QEasingCurve>().type()));

    QtScrollPrepareEvent spe(QPoint(0, 0));
    QApplication::sendEvent(widget, &spe);
...
void SettingsWidget::scrollingCurveChanged(int index)
{
    if (!m_widget || !QtScroller::scroller(m_widget))
        return;

    QtScrollerProperties properties = QtScroller::scroller(m_widget)->scrollerProperties();
    properties.setScrollMetric(QtScrollerProperties::ScrollingCurve,
                                QVariant::fromValue(QEasingCurve(static_cast<QEasingCurve::Type>(m_scrollingCurve->itemData(index).toInt()))));
    QtScroller::scroller(m_widget)->setScrollerProperties(properties);
}

void SettingsWidget::overshootCurveChanged(int index)
{
    if (!m_widget || !QtScroller::scroller(m_widget))
        return;

    QtScrollerProperties properties = QtScroller::scroller(m_widget)->scrollerProperties();
    properties.setScrollMetric(QtScrollerProperties::OvershootCurve,
                                QVariant::fromValue(QEasingCurve(static_cast<QEasingCurve::Type>(m_overshootCurve->itemData(index).toInt()))));
    QtScroller::scroller(m_widget)->setScrollerProperties(properties);
}
...
void SettingsWidget::overshootPolicyChanged(int index)
{
    ...
    if (sender() == m_hospolicy)
        properties.setHorizontalOvershootPolicy(static_cast<QtScrollerProperties::OvershootPolicy>(m_hospolicy->itemData(index).toInt()));
    else if (sender() == m_vospolicy)
        properties.setVerticalOvershootPolicy(static_cast<QtScrollerProperties::OvershootPolicy>(m_hospolicy->itemData(index).toInt()));
    QtScroller::scroller(m_widget)->setScrollerProperties(properties);
}
...
void SettingsWidget::scrollTo()
{
    ...
    if (QtScroller *scroller = QtScroller::scroller(m_widget))
...
{
    QtScroller *s = QtScroller::scroller(m_widget);
    if (!s)
        return;
